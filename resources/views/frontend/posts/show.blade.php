...
@section('content')
      ...
      <div class="content-all w-full min-w-0">
        ...
        <div class="w-full m-auto mb-6 lg:mb-10">
          ...
          <div class="all_category_time_post flex gap-3">
            <div class="terms-links links flex items-center flex-wrap gap-2">
              <span
                class="btn flex-0! btn-primary-2 border-primary/60! py-1.5! px-4! text-[13px]! font-bold shadow-md shadow-primary/20 hover:shadow-primary/60 rounded-full">{{ $post->category?->name ?? 'PHÁP LUẬT MÔI TRƯỜNG' }}</span>
            </div>
          </div>
          ...
          <div
            ...
            <div class="meta flex items-center gap-4 text-xs sm:text-sm text-black">
              <div class="flex items-center gap-1.5 views-svg" itemprop="datePublished">
                ...
                <span
                  class="date">{{ $post->published_at ? $post->published_at->format('d/m/Y') : $post->created_at->format('d/m/Y') }}</span>
              </div>
        ...
        <div class="flex flex-x gap-6 lg:gap-10 items-start">
            ...
            <div class="sidebar-inner">
              <div id="toc_container" role="navigation" aria-label="Table of Contents"
                ...
                <div class="flex items-center justify-between font-bold text-black mb-4 pb-3 border-b border-gray-100">
                  ...
                  <button id="toc-toggle-btn" type="button"
                    ...
                    <svg class="size-3.5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                      stroke-width="3">
          ...
          <div class="content flex-1 min-w-0">
            <article class="entry-content space-y-8 text-black" itemtype="https://schema.org/CreativeWork" itemscope>
              ...
              @if($post->pdf_file)
                <!-- Integrated PDF Document Card & Viewer -->
                <div
                  ...
                  <iframe src="{{ $post->pdf_url }}#toolbar=1" class="w-full border-0 block bg-gray-50"
                    style="height: 650px; min-height: 450px;" title="{{ $post->title }}" loading="lazy">
                    <p class="p-6 text-center text-gray-500 text-sm">
                      Trình duyệt của bạn không hỗ trợ xem PDF trực tiếp.
                      <a href="{{ $post->pdf_url }}" class="text-primary font-bold underline" download>Bấm vào đây để tải
                        file PDF về máy</a>.
                    </p>
                  </iframe>
                </div>
  ...
  <section class="section section-related py-10 lg:py-16 relative overflow-hidden">
        ...
        <div>
          ...
          <div class="inline-flex items-center gap-2 lg:gap-3 mb-2">
            <span class="icon-list-icon">
              <img src="{{ asset("assets/images/asterisk.png") }}" class="size-5" width="24" height="24"
        ...
        @foreach($relatedPosts as $relPost)
          <div
            ...
            <div class="p-thumb c-cover overflow-hidden rounded-2xl relative aspect-16/10">
              <a class="block w-full h-full c-scale-effect" href="{{ route('posts.show', $relPost->slug) }}"
                ...
                <img
                  ...
                  width="1024" height="683" alt="{{ $relPost->title }}" loading="lazy" />
              </a>
              <span
            ...
            <div class="p-content flex flex-col flex-1 justify-between">
              <div>
                <div class="terms mb-3 flex flex-row flex-wrap items-center gap-2">
                  <span
                    class="term btn btn-secondary-2 flex-0! py-1! px-3! text-[12px]! rounded-full">{{ $relPost->category?->name ?? 'Hồ sơ pháp lý' }}</span>
                </div>
                <a class="c-hover block" href="{{ route('posts.show', $relPost->slug) }}" title="{{ $relPost->title }}">
                  <h3
                    class="font-bold text-lg text-black group-hover:text-primary transition-colors leading-snug line-clamp-2">
                    {{ $relPost->title }}
                  </h3>
                </a>
                <p class="mt-2 text-sm text-black line-clamp-2 leading-relaxed">
                  {{ $relPost->excerpt }}
                </p>
              </div>
              <div class="mt-4 pt-3 border-t border-gray-100 flex items-center justify-between gap-2">
                <span
                  class="text-xs text-black font-medium truncate">{{ $relPost->published_at ? $relPost->published_at->format('d/m/Y') : '' }}</span>
                <a href="{{ route('posts.show', $relPost->slug) }}"
